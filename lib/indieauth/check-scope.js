...
module.exports = (requiredScope, tokenScope) => {
  ...
  */
  ...
  // if a token doesn't include a scope, it should default to 'create'
  // so, let's initialize with create
  const scopes = new Array('create');
  
  // if there IS a tokenScope, let's assign it -- otherwise it has create assigned from initialization
  if (tokenScope) {
    scopes = tokenScope.split(' ');
  }
